...
- 🎬 **Сторителлинг через прокрутку** — 9 глав ([`chapters.ts`](src/components/3d/chapters.ts)) с собственными кадрами камеры, заголовками и подписями.
- 🛰️ **Спиральный облёт камеры** — позиция считается по углу/радиусу/высоте (`spiralKey`), даёт ~1.06 оборота вокруг логотипа вместо линейных переходов.
- ✂️ **Переходы «blur-cut»** — резкая склейка со скачком FOV (до ~104°) для имитации motion-blur при входе в туннель и выходе из него.
- 🌀 **Raymarched-туннель** — полноэкранный фрактальный шейдер (Mandelbox-складки + domain warping) с собственной виртуальной камерой; разворачивается в главе 7.
- 🎥 **Кинематографичная cutscene** — при пересечении порога прокрутки страница «пинится» на 7 с, ввод блокируется, а `scrollY` анимируется программно (`CutsceneController`).
- ✨ **Постобработка** — Bloom, Chromatic Aberration, Brightness/Contrast, Vignette, Film Noise через `@react-three/postprocessing`.
- 💸 **Живые данные SOL** — хук `useSolPrice` тянет цену, изменение за 24ч, объём, market cap и 7-дневный sparkline из CoinGecko (обновление раз в минуту).
- 🌐 **3D GLB-логотип** — модель Solana с billboard-разворотом к камере, spring-инерцией под курсор и «дыханием».
- 🎯 **Типографический оверлей** — Framer Motion-анимация заголовков, прогресс глав, hint-прокрутки.
- ♿ **`prefers-reduced-motion`** — отключает анимации и cutscene для пользователей с чувствительностью к движению.
- 📱 **Адаптивный DPR** — `dpr={[1, 1.75]}` балансирует резкость и производительность.
